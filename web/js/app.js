function main() {
    ...
    const topScale = mkVolumeScale(1, 100000, 0, 100);
    const bottomScale = mkVolumeScale(100000, 1, 0, 100);
    const priceScale = mkPriceDeltaScale(book, width - 50);

    const svg = prepareSvg("btc-usd-bitmex", width, height + 50);

    renderVolumeAxis(svg, "offer", topScale, 4, width - 50, 10);
    renderVolumeAxis(svg, "bid", bottomScale, 4, width - 50, height / 2 + 10);
    renderPriceAxis(svg, priceScale, 0, height / 2 + 10);
}
...
function mkPriceDeltaScale(book, width) {
